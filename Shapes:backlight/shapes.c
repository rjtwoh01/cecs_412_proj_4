...
void checkCharacter(const char *character, int str_length);
...
int main(void)
{
	...
	pwm_start(&mypwm[0], 25);
	pwm_start(&mypwm[1],50);
	pwm_start(&mypwm[2],75);
	pwm_start(&mypwm[3],100);
	...
	while (true) {
		...
		if(userInput == 100){
			checkCharacter((char *)str, str_length);
			str_length = 0;
...
void checkCharacter(const char *str , int str_length)
{
	if(strcmp((const char *)str, "BLON") == 0 || str_length == 4){
    gpio_set_pin_high(NHD_C12832A1Z_BACKLIGHT); //turns backlight on
	}

	if(strcmp((const char *)str, "BLOFF") == 0 || str_length == 5){
    gpio_set_pin_low(NHD_C12832A1Z_BACKLIGHT); //turns backlight off
	}

	if(strcmp((const char *)str, "CIRCLE") == 0){
		displayShape(1);
	}

  if(strcmp((const char *)str, "SQUARE") == 0){
    displayShape(2);
  }
}
...
void displayShape(int shape){
  ...
  finished = 1;
  while(finished){
    input2 = usart_getchar(USART_SERIAL_EXAMPLE);
    int userInput = getCharacter(input2);
    if(userInput == 100){
      finished = 0;
    }
  }
  if(shape == 1){
    gfx_mono_generic_draw_filled_circle(input, input2, 1, )
  }
  if(shape == 1){
    gfx_mono_generic_draw_filled_circle(input, input2, )
  }
}
