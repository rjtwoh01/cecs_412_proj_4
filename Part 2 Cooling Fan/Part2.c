...
{
	if(strcmp((const char *)str, "HIGH") == 0 || str_length == 4){
		pwm_start(&mypwm[0], 95);
	}
	
	if(strcmp((const char *)str, "LOW") == 0 || str_length == 3){
		pwm_start(&mypwm[0], 50);
	}

	if(strcmp((const char *)str, "MEDIUM") == 0 || str_length == 6){
		pwm_start(&mypwm[0], 75);
	}	
}
